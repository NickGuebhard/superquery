select ga.product as prod, ga.platform as plat, ga.mth as mth, sum(ga.transactions) as transactions, sum(ads.cost) as ads_cost, sum(fb.cost) as fb_cost
from
...
(SELECT 'BBOX' as product, sum(cost) as cost, 'Google Ads' as platform, substr(cast(date as string), 5, 2) as mth --count(distinct date) as dates 
FROM `d3-projects-271420.MFB_extraction.ads_bbox_ext`
where date >= '2020-01-01'
group by date
union all
SELECT 'MFB FS' as product, sum(cost) as cost, 'Google Ads' as platform, substr(cast(date as string), 5, 2) as mth --count(distinct date) as dates 
FROM `d3-projects-271420.MFB_extraction.ads_mfb_fs_ext`
where date >= '2020-01-01'
group by date
union all
SELECT 'MADE' as product, sum(cost) as cost, 'Google Ads' as platform, substr(cast(date as string), 5, 2) as mth --count(distinct date) as dates 
...
(SELECT 'BBOX' as product, sum(spend) as cost, 'Facebook Ads' as platform, substr(cast(date_start as string), 5, 2) as mth --, count(distinct date_start) as dates 
FROM `d3-projects-271420.MFB_extraction.fb_bbox_ext`
where date_start >= '2020-01-01'
group by date_start
union all
SELECT 'MFB FS' as product, sum(spend) as cost, 'Facebook Ads' as platform, substr(cast(date_start as string), 5, 2) as mth--, count(distinct date_start) as dates 
FROM `d3-projects-271420.MFB_extraction.fb_mfb_fs_ext`
where date_start >= '2020-01-01'
group by date_start
union all
SELECT 'MADE' as product, sum(spend) as cost, 'Facebook Ads' as platform, substr(cast(date_start as string), 5, 2) as mth--, count(distinct date_start) as dates 
